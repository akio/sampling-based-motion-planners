...
RrtStar::RrtStar(SpacePtr space, NearestNeighborPtr nn) : nn_(nn), space_(space) {}
...
/// [1] S. Karaman and E. Frazzoli, “Sampling-based Algorithms for Optimal Motion Planning.”
RrtStar::Status RrtStar::extend(const NodePtr& x_sample, NodePtr& x_new_out) {
  ...
  if (x_new) {
    ...
    std::cout << "radius = " << radius << ", near nodes = " << x_near_vec.size() << std::endl;
    ...
    for (auto& x_near : x_near_vec) {
      ...
      std::cout << "  cost = " << cost << " min_cost = " << min_cost
                ...
                << ")  x_near = (" << std::static_pointer_cast<Motion2D>(x_near.first)->x
    ...
    for (auto& x_near : x_near_vec) {
      ...
      std::cout << "  cost = " << cost << " current_cost = " << current_cost << std::endl;
      if (cost < current_cost && space_->collision_free_path(x_near.first, x_new)) {
...
/// [1] B. Akgun and M. Stilman, “Sampling Heuristics for Optimal Motion Planning in High Dimensions,”
