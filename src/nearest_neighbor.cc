...
std::vector<NearestNeighborInterface::NodeWithCost> LinearSearchNN::nearest_neighbors(NodePtr node, size_t k) const {
  ...
  std::sort(begin(work), end(work), [](const NodeWithCost& a, const NodeWithCost& b) {
...
std::vector<NearestNeighborInterface::NodeWithCost> LinearSearchNN::radius_search(NodePtr node, double radius) const {
  assert(radius >= 0);
  if (radius == 0) {
    return {};
  }
